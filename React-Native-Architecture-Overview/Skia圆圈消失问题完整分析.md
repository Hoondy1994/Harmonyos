...
**用户反馈：**
- 进入 Details 页面，再切回 Details，圆圈不显示了

**复现步骤：**
1. 打开应用，进入 Goods 页面
2. 点击"跳转 Details1"，进入 Details 页面
3. 观察：Skia Canvas 绘制的三个圆圈（cyan、magenta、yellow）正常显示 ✅
4. 返回 Goods 页面
5. 再次点击"跳转 Details1"，进入 Details 页面
6. 观察：圆圈消失，页面其他内容（文字、按钮）正常显示 ❌

### 问题影响

- **功能影响**：Skia Canvas 组件无法正常显示
- **用户体验**：页面内容不完整，影响使用
- **影响范围**：所有使用 `BasePrecreateView` 且包含 Skia Canvas 的页面
...
**skia_circle_fix.patch** - 包含所有修改的 patch 文件
...
初步分析（可能的原因）
  ↓
添加日志验证（确认 Skia 组件没有重新创建）
  ↓
分析 BasePrecreateView 执行流程（发现预创建机制）
  ↓
理解预创建机制（后台运行 bundle，没有 surface）
  ↓
确认根因（Skia 需要 attach 到 surface，但预创建时没有）
  ↓
探索解决方案（强制重新运行 bundle）
  ↓
实现修复（添加 forceReloadOnAppear 开关）
  ↓
修复 Detail2 跳转路径问题
  ↓
验证修复（圆圈正常显示）
