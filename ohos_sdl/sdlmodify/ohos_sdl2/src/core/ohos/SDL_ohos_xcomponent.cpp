...
SDL_Window *GetWindowFromXComponent(OH_NativeXComponent *component)
{
...
static void OnSurfaceChangedCB(OH_NativeXComponent *component, void *window)
{
    ...
    double offsetY;
    OH_NativeXComponent_GetXComponentSize(component, window, &width, &height);
    g_ohosSurfaceWidth = width;
    g_ohosSurfaceHeight = height;
    OH_NativeXComponent_GetXComponentOffset(component, window, &offsetX, &offsetY);
    ...
    if (curWindow && g_ohosResizeSync) {
        SDL_LockMutex(g_ohosResizeSync->sizeChangeMutex);
        OHOS_SendResize(curWindow);
        g_ohosResizeSync->isChangeing = false;
        SDL_CondSignal(g_ohosResizeSync->sizeChangeCond);
...
void OnUIInputEventCB(OH_NativeXComponent *component, ArkUI_UIInputEvent *event, ArkUI_UIInputEvent_Type type)
{
    if (type != ARKUI_UIINPUTEVENT_TYPE_AXIS) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "not axis events are not handled.");
        return;
    }
    ...
    OHOSMouseAxisData mouseAxisData;
    SDL_LockMutex(g_ohosPageMutex);
    mouseAxisData.wheelX = horizontalAxisValue;
    mouseAxisData.wheelY = verticalAxisValue;
    SDL_Window *curWindow = GetWindowFromXComponent(component);
    if (curWindow == nullptr) {
    ...
    OHOS_OnMouseAxis(curWindow, &mouseAxisData);
    SDL_UnlockMutex(g_ohosPageMutex);
}
...
void OHOS_XcomponentExport(napi_env env, napi_value exports)
{
    ...
    if (napi_ok != napi_unwrap(env, exportInstance, (void **)(&nativeXComponent))) {
        SDL_Log("Xcompent js callback is coming, napi_unwrap111111111111222222222222 id is111111111");
        return;
    }
    ...
    SDL_Log("Xcompent js callback is coming, xcompent id is111111111 %s.", xComponentId.c_str());
    ...
    OH_NativeXComponent_RegisterUIInputEventCallback(nativeXComponent, OnUIInputEventCB, ARKUI_UIINPUTEVENT_TYPE_AXIS);
    return;
}

void OHOS_SetNativeXcomponent(char *xcompentId, void *native)
{
    OH_NativeXComponent *nativeXComponent = (OH_NativeXComponent *)native;

    std::string xComponentId(xcompentId);
    if (xComponentId.empty()) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "get xComponent error");
        return;
    }
